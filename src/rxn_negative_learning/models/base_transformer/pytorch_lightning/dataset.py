...
from rxn_negative_learning.utils.smiles_utils import randomize_multiple_smiles_rotated
...
class SmilesDataset(Dataset):
    ...
    def __init__(
        ...
        logger.info(f"Smiles dataset length: {self.length}")
        self.padding_idx = padding_idx
        self.augment = augment and is_training_dataset
    ...
    def __getitem__(self, index) -> Dict:
        ...
        Returns:
            tokenized item.
        """
        # Note: only int-like objects can be saved in the item

        examples = self.examples_reader()
        example = examples[index]
        item = {}

        augment = self.augment and random.choice([True, False])
        if augment:
            augmented_smiles = randomize_multiple_smiles_rotated(example["source"])
            source_item = self.tokenizer(augmented_smiles)
            item["source_augmented"] = augmented_smiles
        else:
        ...
        if "score" in example.keys():
            ...
            item["idx"] = example["idx"]

        item["source"] = example["source"]
        item["target"] = example["target"]

        item["encoder_input_ids"] = source_item["input_ids"]
        item["encoder_padding_mask"] = source_item["attention_mask"].eq(self.padding_idx)

        item["decoder_input_ids"] = target_item["input_ids"]
        item["decoder_padding_mask"] = target_item["attention_mask"].eq(self.padding_idx)
        return item
...
class LitSmilesDataset(pl.LightningDataModule):
    ...
    def __init__(self, dataset_args: Dict[str, Any], tokenizer: SmilesTokenizer) -> None:
    ...
    def build_dataset(
        self, path: Union[str, PosixPath], is_training_dataset: bool = False
    ) -> Dataset:
        """
        Build the dataset.
        ...
        Args:
            ...
            is_training_dataset: if the dataset will be used for training
    ...
    def load(self) -> None:
        ...
        self.datasets = {
            "train": self.build_dataset(self.dataset_args["train_file"], is_training_dataset=True),
            "validation": self.build_dataset(self.dataset_args["validation_file"]),
            "test": self.build_dataset(self.dataset_args["test_file"]),
        }

        logger.info(
            f"Training set size: {len(self.datasets['train'])} - Validation set size: {len(self.datasets['validation'])} - Test set size: {len(self.datasets['test'])}"
            # type: ignore
        )
    ...
    def train_dataloader(self) -> DataLoader:
        ...
        return DataLoader(
            ...
            shuffle=True,  # default is false!! Controls if we shuffle after each epoch
            worker_init_fn=seed_worker,  # check now that the shuffling is done but always in the same order
            generator=g,
        )

    def val_dataloader(self) -> DataLoader:
        """Create the DataLoader for the validation step.
    ...
    def test_dataloader(self) -> DataLoader:
        """Create the DataLoader for the test step.
    ...
    def add_dataset_specific_args(parent_parser: ArgumentParser) -> ArgumentParser:
        """Adds dataset specific arguments to the parser.
